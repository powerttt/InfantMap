...
public class BusPathSearch {
    ...
    Logger log=Logger.getLogger(BusPathSearch.class);
    ...
    @RequestMapping("/saveInRedis")
    public void saveSiteNameMapRedis(){
        if (busPathService.saveSiteNameMapRedis()){
            log.info("成功备份到Redis中!");
        }else {
            log.debug("备份失败");
        }
    }
